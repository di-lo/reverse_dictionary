...
def reverse(original):
    ...
    """
    reversed_dict = {} # empty dictinory
    for old_key in original:
        old_value = original[old_key] # keys are values and values are keys -> old_value : old_key -> ages : names
        ...
        if old_value not in reversed_dict:
            ...
            reversed_dict[old_value] = [old_key] # make a list by [] # age associated with list of one name
        else:
